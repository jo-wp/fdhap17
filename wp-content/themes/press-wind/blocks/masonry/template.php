...
<section <?= get_block_wrapper_attributes(["class" => 'block-mansonry max-w-[1100px] mx-auto ']); ?>>
  ...
  <div class="md:grid grid-mansonyried max-md:grid-cols-2 max-md:flex max-md:overflow-x-scroll max-md:gap-[15px] max-md:ml-[15px]">
    <?php foreach ($items_associated as $post):
      $featuredImage = get_the_post_thumbnail_url($post->ID, 'full');
    ?>
      <article class="post relative after:rounded-[20px] after:z-10 after:w-full after:h-full after:absolute after:content-[''] after:top-0 after:left-0 min-h-[430px] rounded-[20px] bg-cover bg-center max-md:min-w-[340px]" style="background-image: url('<?= esc_url($featuredImage); ?>');">
